...
public:
  ...
  void runVocab(const std::string &name, 
    const FeatureExtractor<TDescriptor> &extractor,std::vector<std::vector<FSurf64::TDescriptor > > &features);
...
//here set dislocal
  ...
  for(unsigned int i = 0; i < filenames.size(); ++i)
  {
	...
	mythread_comm.isamupdating=true;
        ss = my_for_g2o_edge(i+1,result.match+1,myviso.relate,isam2,true,nfg);
...
template<class TVocabulary, class TDetector, class TDescriptor>
void demoDetector<TVocabulary, TDetector, TDescriptor>::runVocab
  (const std::string &name, const FeatureExtractor<TDescriptor> &extractor,std::vector<std::vector<FSurf64::TDescriptor > > &features)
{
  namedWindow( "Display window", WINDOW_AUTOSIZE );
  cout << "LoopDetector" << endl;
